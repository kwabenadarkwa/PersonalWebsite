...
#### Event Pipeline(Now Volnux) - Side Story But Relevant to Agean
Over the course of this I met someone who introduced me to this open-source tool called Event Pipeline(now Volnux[^2]). I realised after reading through the docs that I could use it for Agean because it would make building the various blocks of the entire pipeline isolated and easily testable. That is in terms of finding out if optimizations that I did as can be seen in the pipeline diagram above actually made any real improvements in terms of speed most importantly. The Batching feature that Nexus has I thought would also help me when it comes to bulk testing

Volnux has a DSL(Domain Specific Language) called pointy-lang that allows users to "script" the order in which "events"(anything you want to happen) should be triggered. For instance, if I created two events say *Fetch* and *Decode*,I could use pointy-lang to write the following(this is not an exhaustive list): 
...
Volnux also exposes a *monitor_events()* api[^3] that allows users to monitor telemetry about the various events in the pipeline that they had built. 
...
### Foreshadowing
In the end I got the award for the best research project for my class and I'm really happy about that. 
<div className="flex justify-center">
...
[^2]:  https://github.com/nshaibu/volnux
[^3]:  This is was my first contribution to Volnux as it wasn't fully built and had a few bugs when I had a need for it. 
